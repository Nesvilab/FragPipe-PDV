...
public class RealTimeExportJDialog extends JDialog {
    ...
    public RealTimeExportJDialog(Integer forwardIon, Integer rewindIon, Integer height, Integer width, String unit
            , PeptideSpectrumAnnotator peptideSpectrumAnnotator, SpecificAnnotationSettings specificAnnotationSettings, GUIMainClass guiMainClass
        ...
        this.annotationSettings = guiMainClass.getAnnotationSettings();
                        ...
                        for (ArrayList<String> eachPage : allSpectrums) {
                            for (String spectrumKey : eachPage) {
                                if (!progressDialog.isRunCanceled()) {
                                    ...
                                    mSnSpectrum = guiMainClass.getSpectrumRealTime(spectrumKey);
                                    spectrumIdentificationAssumption = guiMainClass.getspectrumIdentificationAssumption();

                                    Thread.sleep(100);
                                    updateSpectrum(spectrumKey, mSnSpectrum, spectrumIdentificationAssumption);

                                    String outputPathName = outputPath + GUIMainClass.FILE_SEPARATOR + FilePattern.matcher(mSnSpectrum.getSpectrumTitle()).replaceAll("") + "_" + currentPeptide + imageType.getExtension();
                    ...
                    if(resizeJPanelWidth < 200){
                        ...
                        for (String eachKey : allSelections) {
                            if (!progressDialog.isRunCanceled()) {
                                ...
                                mSnSpectrum = guiMainClass.getSpectrumRealTime(eachKey);
                                spectrumIdentificationAssumption = guiMainClass.getspectrumIdentificationAssumption();
